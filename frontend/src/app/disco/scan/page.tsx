...
export default function ScanDiskPage() {
  ...
  const handleSubmit = async (requests: number[], head: number, direction?: 'up' | 'down') => {
    setLoading(true); setResult(null);
    try {
      const res = await fetch('http://localhost:3015/algorithms/disco/scan', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ requests, head, direction, maxCylinder: 199 }),
      });
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-white mb-2">SCAN Disco — Algoritmo del Ascensor</h1>
        <p className="text-gray-400">
          El cabezal se mueve en una dirección atendiendo solicitudes hasta llegar al extremo del disco, luego invierte su dirección y atiende las restantes.
        </p>
      </div>
      
      <DiskSchedulingForm onSubmit={handleSubmit} loading={loading} needsDirection={true} />
