...
export default function CScanDiskPage() {
  ...
  const handleSubmit = async (requests: number[], head: number) => {
    ...
    try {
      // C-SCAN generally moves in one direction (up) according to the service logic we implemented.
      const res = await fetch('http://localhost:3015/algorithms/disco/c-scan', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ requests, head, maxCylinder: 199 }),
      });
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-white mb-2">C-SCAN Disco — Circular SCAN</h1>
        <p className="text-gray-400">
          El cabezal se mueve en una sola dirección hasta el extremo del disco. Al llegar, retorna inmediatamente al cilindro 0 sin atender solicitudes en el camino de vuelta.
        </p>
      </div>
      
      {/* We do not need direction for C-SCAN since it's generally implemented moving towards max cylinder by default */}
      <DiskSchedulingForm onSubmit={handleSubmit} loading={loading} needsDirection={false} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden flex flex-col">
              <div className="p-4 bg-gray-900 border-b border-gray-700">
                ...
                <p className="text-xs text-gray-500 mt-1">El salto de regreso al inicio (0) suma al recorrido total.</p>
              </div>
