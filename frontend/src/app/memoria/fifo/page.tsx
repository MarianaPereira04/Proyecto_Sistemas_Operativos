...
export default function FifoPage() {
  ...
  const handleSubmit = async (pages: number[], frames: number) => {
    ...
    try {
      const res = await fetch('http://localhost:3015/algorithms/memoria/fifo', {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-white mb-2">FIFO — First In, First Out</h1>
        <p className="text-gray-400">Reemplaza la página que lleva más tiempo cargada en memoria (la primera en entrar es la primera en salir).</p>
      </div>
      ...
      {result && (
        <div className="space-y-6">
          ...
          <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
            ...
            <PageStepsTable steps={result.steps} frames={frameCount} />
          </div>
        </div>
      )}
      {!result && !loading && (
