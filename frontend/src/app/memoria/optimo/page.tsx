...
export default function OptimoPage() {
  ...
  const handleSubmit = async (pages: number[], frames: number) => {
    ...
    try {
      const res = await fetch('http://localhost:3015/algorithms/memoria/optimo', {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-white mb-2">Óptimo (Optimal)</h1>
        <p className="text-gray-400">
          Reemplaza la página que <strong className="text-white">no será usada por el mayor tiempo en el futuro</strong>. 
          Produce la menor tasa de fallos posible, pero requiere conocimiento previo de las referencias.
        </p>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
            <div className="p-4 bg-gray-900 border-b border-gray-700">
              ...
              <p className="text-xs text-gray-500 mt-1">Al producirse un fallo, se reemplaza la página cuyo próximo uso está más lejano en la cadena (o que no volverá a usarse).</p>
            </div>
            <PageStepsTable
              steps={result.steps}
              frames={frameCount}
              showExtra={(step) =>
                step.fault && step.replaced !== null ? (
                  <span>Reemplazó: <strong className="text-orange-400">{step.replaced}</strong> (uso más lejano)</span>
                ) : null
              }
            />
          </div>
